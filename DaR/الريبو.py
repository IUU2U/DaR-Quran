...
from DaR.helpers.decorators import authorized_users_only
...
async def restart(client, m: Message):
    ...
    await loli.edit("8")
    await loli.edit("9")
    await loli.edit("**✅ تـم اعـاده تشغـيل دار ميـوزك**")
    os.execl(sys.executable, sys.executable, *sys.argv)
    quit()
@Client.on_message(filters.user(SUDO_USERS) & filters.command(["اوامر","اوامر اغاني"], prefixes=f"{HNDLR}"))
@authorized_users_only
...
⧉ | لتشغيل صوتية في المكالمة أرسل ⇦ [ `{HNDLR}تشغيل  + اسم الاغنيه + او الرابط من يوتيوب` ]
...
———————×———————
 مطــور ســورس دار : @S88F8
قنـاه ســورس دار:  @IUU2U"""
    await m.reply(HELP)
