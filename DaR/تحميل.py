...
async def song(client, message: Message):
    ...
    if not urlissed:
        ...
        return
    pablo = await client.send_message(message.chat.id, f"**🔎 جاري تحميل : @SlSI0 ** `{urlissed}`")
...
async def vsong(client, message: Message):
    ...
    pablo = await client.send_message(message.chat.id, f"**🔎 يبحث عن : @SlSI0** `{urlissed}`")
    if not urlissed:
    ...
    try:
        ...
        await event.edit(event, f"**التحميل فشل : @SlSI0** \n `{str(e)}`")
        return
    c_time = time.time()
